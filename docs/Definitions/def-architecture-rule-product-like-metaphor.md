---
aliases: []
date: '2026-09-11T00:43:14.909630'
meeting_id: e9fc736f
project: Asteroids Survivor
summary: Room consensus after reviewing the operator's choice.
tags:
- incubator
- decision
- definition
- architecture
title: 'Rule: What is this product like, as a metaphor we can all share?'
type: definition
---

# Rule: Product Metaphor Definition

This note records the agreed-upon metaphor that guides product design and serves as a shared reference for the build team.

## Definition

**A space you inhabit** — The product reads as a world you are inside: things move around you and you act in place.

## Rationale

Room consensus after reviewing the operator's choice.

## Trade-offs / Key Considerations

### Advantages
- Aligned with the goals the operator stated
- Clear direction for the build team

### Drawbacks / Constraints
- Constrains the design to the selected option

### Risks to Watch
- Needs to be validated during the MVP build

## Links

- [[Project]]
- [[Index]]
- [[2026-09-11-plan-first-increment-ordered]]
- [[2026-09-11-plan-first-increment-ordered-minutes]]
